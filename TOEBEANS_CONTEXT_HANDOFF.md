...
* Phase 3 / M2 (vertical slice — polish ONE area end to end): IN PROGRESS.
  ...
  - [ ] Remaining M2 items: sound (music + effects, next), save/load, character art (skier/cat models under the assets item).
  - Director instruction (July 21, 2026): **re-ask the open direction questions at the top of the next session** — sound/music direction, plus ratify-or-change on the real-UI session's three defaults (HUD-only scope, nine icons, soft-rounded tone). Cat-model direction also still open.
