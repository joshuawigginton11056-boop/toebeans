...
* Phase 3 / M2 (vertical slice — polish ONE area end to end): IN PROGRESS.
  ...
  - [ ] Remaining M2 items, in the director's order: save/load (next), character art + remaining slope assets (skier/cat models), then music (timed per-slope songs — deliberately last), then the end-of-M2 tuning pass (parked picky tweaks + carve volume).
  ...
  - Claude Code's browser screenshots have timed out in every session in this repo (eight and counting); logic gets verified by stepping the real modules in the live page or checking computed styles, and rendered-look judgments go to the director's own eyes via `npm run dev`.
...
* Art assets DO exist in the repo now (24 CC0 .glb models in assets/slope/, all credited in assets/CREDITS.md) and the style bible IS written (DESIGN.md). A real HUD (cat-face lives, banners, keycap hints — restyled to the middle-ground tone) and slope sound effects (synthesized, no files) exist too. XP, music, and save/load still do not exist.
